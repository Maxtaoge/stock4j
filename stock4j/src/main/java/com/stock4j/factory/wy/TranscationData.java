...
class TranscationData extends HttpClientPool {
    ...
    /**
     * 获取当日的分时图数据 http://img1.money.126.net/data/hs/time/today/0600510.json
     *
     * @param stock
     * @return
    ...
    /**
     * 从网易取分时数据 http://img1.money.126.net/data/hs/time/4days/0600510.json
     *
     * @param stock 股票，起始时间
     * @return
     * @throws ErrorHttpException
        ...
        try {
            ...
            for (int i = node.size() - 1; i >= 0; i--) {
                ...
                for (int j = 0, len = dataBody.size(); j < len; j++) {
					...
					if (dateTime != null && dateTime.isBefore(sdate)) {
						continue;
					}
