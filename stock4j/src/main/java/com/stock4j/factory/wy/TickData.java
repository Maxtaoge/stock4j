...
class TickData extends HttpClientPool {
    ...
    /**
     * 获取股票的日线数据，获取数据的数据可以很大 http://img1.money.126.net/data/hs/kline/day/history/2017/0600510.json
     * http://img1.money.126.net/data/hs/klinederc/day/history/2017/0600510.json 后复权
     * http://img1.money.126.net/data/hs/klinederc/week/history/2017/0600510.json 周线 后复权
     * http://img1.money.126.net/data/hs/klinederc/month/history/2017/0600510.json 月线 数据按时间升序
     */
    protected List<Tick> listTicks(Stock stock, PeriodType period, int size, ExRightType rhb)
        ...
        do {
            String url =
                "http://img1.money.126.net/data/hs/" + rhbType + "/" + ptype + "/history/" + date.getYear() + "/"
                    + scode + ".json";
            try {
		...
		if (size >= realLen) {
			...
			return ticks.subList(realLen - size, realLen);
		}
    }
